...
public class LowGravityPhysics implements BallPhysicsStrategy {
    ...
    public void update(Ball ball, int speedfactor) {
        ...
        if (ball.getY() > lowBound) {
            if (velX > 0) ball.setVelX(velX - 0.15);
            else if (velX < 0) ball.setVelX(velX + 0.15);
            if (Math.abs(ball.getVelX()) < 0.15) ball.setVelX(0);
        }
    }
}
